...
figma.ui.onmessage = (msg) => __awaiter(this, void 0, void 0, function* () {
    ...
    if (msg.type === 'calculate-grid') {
        ...
        if (columns <= 0) {
            figma.ui.postMessage({ type: 'grid-results', data: { calculatedPageWidth: 0, columnWidth: 0, color: 'var(--figma-color-text-danger)' } });
            return;
        }
    ...
    if (msg.type === 'generate-actions') {
        ...
        let createdOrUpdatedVariables = null;
        ...
        try {
            ...
            if (generateVariables) {
                ...
                const existingVariablesInGroup = allVariables.filter(v => v.variableCollectionId === collection.id && v.name.startsWith(groupPrefix));
                createdOrUpdatedVariables = new Map();
                ...
                const desiredVariables = new Map();
...
function createGridFrame(params) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const fillPaint = { type: 'SOLID', color: gridColor, opacity: 0.08 };
        if (variables) {
        ...
        if (variables) {
            // BOUND MODE
            layoutGrid = {
                pattern: 'COLUMNS', alignment: 'STRETCH', count: columns, color: Object.assign(Object.assign({}, gridColor), { a: 0.08 }),
                gutterSize: variables.get('gutter').resolveForConsumer(frame).value,
                offset: variables.get('margin').resolveForConsumer(frame).value,
                boundVariables: {
            ...
            layoutGrid = {
                ...
                gutterSize: gutter, offset: margin, color: Object.assign(Object.assign({}, gridColor), { a: 0.08 })
            };
        }
